...
def validate_time_period(data, time_period):
    assert data.size(0) >= time_period, "Time period should be smaller than data size"
...
def generate_train_and_test_sets(data, time_period, train_proportion=0.75, discrete_targets=False):
    ...
    :param data: size = (time period, nb features observed at time t)
    ...
    nb_samples = data.size(0) - time_period
    ...
    train_set = torch.Tensor([[data[idx + t].tolist() for t in range(time_period)] for idx in train_indices])
    test_set = torch.Tensor([[data[idx + t].tolist() for t in range(time_period)] for idx in test_indices])
    ...
    if discrete_targets:
        ...
        train_targets = torch.Tensor([generate_target(data[idx + time_period - 1, 0].tolist(),
                                                      data[idx + time_period, 0].tolist())
                                      for idx in train_indices])
        test_targets = torch.Tensor([generate_target(data[idx + time_period - 1, 0].tolist(),
                                                     data[idx + time_period, 0].tolist())
